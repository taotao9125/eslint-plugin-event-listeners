    ...
    isListenerClassMethod(node) {
        ...
        if (arg) {
            ...
            if (callbackType  === 'MemberExpression') {
                return true;
            } else if (callbackType === 'CallExpression') {
                // callback is a classMethod but use this.method.bind()
                if(this.getServiceMethodName(arg) === 'bind' 
                    && arg.callee.object && arg.callee.object.type === 'MemberExpression') {
                    return true;
                }
            }
        }
    ...
    getListenerCallbackName(node) {
        ...
        if (arg && this.isListenerClassMethod(node)) {
            ...
            if (callbackType  === 'MemberExpression') {
                ...
                if(this.getServiceMethodName(arg) === 'bind' 
                    && arg.callee.object && arg.callee.object.type === 'MemberExpression') {
                    return arg.callee.object.property.name;
                }
            }
        }
    },
